...
export default function CoachTrainingPackagesPage() {
  ...
  const confirmArchive = async () => {
    ...
    try {
      ...
      showToast("Đã lưu trữ gói tập.");
      refetch();
    } catch (e) {
      setArchiveError(messageForApiError(e));
    } finally {
      setArchiving(false);
    }
  };
  ...
  return (
    <AppShell role="coach" title="Gói tập">
      <div className="max-w-[1040px] mx-auto pb-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-body-sm text-on-surface-variant">
                Tạo gói tập và gửi quản trị viên duyệt. Chỉ gói đã xuất bản mới
                hiển thị công khai.
              </p>
            </div>
          </div>
        ...
        <AnimatePresence>
          {toast && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -6 }}
              role="status"
              className="mb-4 flex items-center gap-2.5 rounded-[12px] border border-[#bce8c8] bg-success-container/50 px-4 py-3 text-[13px] font-medium text-[#1f7a4d]"
            >
        ...
        {/* Filter tabs */}
        <div className="inline-flex flex-wrap items-center gap-1 p-1 bg-surface-container-low rounded-[10px] mb-4">
          {TABS.map((t) => {
              ...
              >
                {active && (
                  <motion.span
                    ...
                    className="absolute inset-0 rounded-[7px] bg-surface-container-lowest shadow-[0_1px_2px_rgba(15,15,30,0.06)]"
                    transition={{ type: "spring", duration: 0.4, bounce: 0.15 }}
                  />
                )}
                <span className="relative">{t.label}</span>
                <span className="relative text-[10px] tabular-nums px-1.5 rounded-full bg-surface-container-high text-on-surface-variant">
                  {count}
                </span>
              </button>
            );
        ...
        {archiveTarget && (
          <Modal onClose={() => !archiving && setArchiveTarget(null)}>
            ...
            <p className="text-[13px] text-on-surface-variant mt-1.5">
              Bạn có chắc muốn lưu trữ “{archiveTarget.title}” không? Gói đã lưu
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              <button
                ...
                disabled={archiving}
                className="inline-flex items-center gap-1.5 h-10 px-4 rounded-[9px] bg-primary text-on-primary text-[13px] font-semibold hover:bg-[#2d20b8] transition-colors disabled:opacity-60"
              >
                {archiving ? (
                  <>
...
function PackageCard({
  ...
  return (
    <article className="rounded-[12px] border border-[var(--color-border-soft)] bg-surface-container-lowest p-4 flex flex-col">
      <div className="flex items-start justify-between gap-2 mb-2">
        <h3 className="text-[15px] font-semibold leading-snug">{pkg.title}</h3>
        <span
          className={cn(
            "shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-[10.5px] font-semibold border",
      ...
      {(pkg.level || pkg.goalType || pkg.location) && (
        <div className="flex flex-wrap gap-1.5 mt-2.5">
          {[pkg.goalType, pkg.level, pkg.location]
            .filter(Boolean)
            .map((tag, i) => (
              <span
                key={`${tag}-${i}`}
                className="px-2 py-0.5 rounded-full bg-surface-container-low text-[11px] text-on-surface-variant"
              >
                {tag}
              </span>
            ))}
      ...
      {(pkg.status ?? "").toLowerCase() === "rejected" && pkg.rejectionReason && (
        <div className="mt-3 rounded-[8px] border border-[#ffbbb3] bg-[#ffdad6]/50 px-3 py-2 text-[11.5px] text-[#ba1a1a]">
          <span className="font-semibold">Lý do từ chối: </span>
          {pkg.rejectionReason}
        </div>
      )}

      <div className="mt-3 pt-3 border-t border-[var(--color-border-soft)] flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="inline-flex items-center gap-1 h-8 px-2.5 rounded-[7px] text-[12px] text-on-surface-variant hover:bg-surface-container-low hover:text-primary transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-on-surface-variant"
          >
            <Pencil size={13} />
            Sửa
          </button>
          {!archived && (
            <button
              onClick={onArchive}
              className="inline-flex items-center gap-1 h-8 px-2.5 rounded-[7px] text-[12px] text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface transition-colors"
            >
              <Archive size={13} />
              Lưu trữ
            </button>
