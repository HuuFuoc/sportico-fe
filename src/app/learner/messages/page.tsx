...
export default function LearnerMessagesPage() {
  ...
  const threads = getThreadsForUser(learnerId);
  ...
  const otherId = active?.participantIds.find((id) => id !== learnerId) ?? "";
  const other = active?.isAI
    ? {
  ...
  return (
    <AppShell role="learner" title="Messages">
      <div className="grid grid-cols-1 md:grid-cols-[320px_1fr] gap-4 h-[calc(100vh-7rem)] max-w-[1400px]">
        {/* Thread list */}
        <aside className="bg-surface-container-lowest border border-[var(--color-border-soft)] rounded-[10px] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {threads.map((t) => {
              const oid =
                t.participantIds.find((id) => id !== learnerId) ?? "";
            ...
            <>
              <header className="px-4 py-3 border-b border-[var(--color-border-soft)] flex items-center gap-3">
                ...
                >
                  ...
                  ) : other && "avatarUrl" in other && other.avatarUrl ? (
                    <img
                      src={other.avatarUrl}
                      alt={other.name}
                      className="w-full h-full object-cover"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-surface-container-low/40">
                {messages.map((m) => {
                  const fromMe = m.senderId === learnerId;
                  return (
                    <div
                      key={m.id}
                      className={cn(
